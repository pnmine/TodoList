<template>
  <div v-if="show" class="modal">
    <div class="modal-content">
      <h2>Edit Category</h2>
      <form @submit.prevent="editCategory(cetegoryItem)">
        <div class="form-group">
          <label for="categoryName">Category Name:</label>
          <input
            type="text"
            id="categoryName"
            v-model="cetegoryItem.name"
            required
          />
...
export default {
  ...
  methods: {
    async editCategory(cetegoryItem) {
      this.validationError = null;

      if (cetegoryItem.name.trim() == "") {
        this.validationError = "Category name is required";
        return;
      }

      try {
        const response = await this.axios.put("/Category", cetegoryItem);
